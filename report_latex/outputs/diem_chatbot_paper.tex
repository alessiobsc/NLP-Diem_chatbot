...
\textbf{Embedding e Reranking.} Due modalità selezionabili via \code{EMBEDDING\_PROVIDER}:
locale con \texttt{intfloat/multilingual-e5-base} (prefissi \texttt{query:}/\texttt{passage:})
o cloud con \texttt{qwen/qwen3-embedding-8b} via OpenRouter (4096 dimensioni, query
instruction esplicita). \texttt{baai/bge-m3} è stato valutato ma scartato perché Chroma
usa ricerca densa pura, senza sfruttare le componenti sparse/ColBERT del modello. Il
reranking è dual-provider: \texttt{BAAI/bge-reranker-v2-m3} locale (singleton) o
\texttt{Cohere Rerank~4 Pro} (32K contesto) via OpenRouter.
...
supporta profondità di reasoning configurabile, chain-of-thought completo e tool use
nativo (function calling, structured output) --- la combinazione di potenza di un
...
\code{qwen2.5:7b} locale, a una fase intermedia con due modelli distinti (32B routing
$+$ 9B risposta), fino al singolo \code{gpt-oss-120b} finale, più stabile nel ciclo
rewrite--retrieve--answer. L'enrichment (context header) usa \code{mistralai/mistral-nemo}
via OpenRouter come modello primario storico, reso configurabile tramite
\code{OPENROUTER\_CONTEXT\_HEADER\_MODEL} per allineare le run finali su
\code{llama-3.1-8b-instruct}.
...
crawling, parsing/filtraggio, arricchimento contestuale e indicizzazione. Un'analisi
manuale preliminare ha identificato i tre domini che costituiscono la knowledge base:
...
(programmi, materiale didattico), con strutture di navigazione e CMS eterogenei che
richiedono strategie di crawling differenziate.
...
di EasyCourse per calendario esami e orario lezioni dei cinque corsi DIEM, un documento
per coppia (CDL, insegnamento) che evita la frammentazione per singolo appello; vengono
inoltre generati gli URL dei bandi DIEM (8 categorie) e scaricate le sottopagine
...
\code{favor\_precision=True}), scelta supportata dallo studio comparativo con Crawl4AI
(Sezione~\ref{sec:migration}: $+$0,097 di overlap lessicale su fatti atomici). I PDF usano
...
Poiché i CMS non espongono metadati temporali (zero segnali su 30 pagine campionate), il
filtro \code{should\_skip\_url\_by\_year} applica regole differenziate per categoria,
sostituendo un cutoff fisso a 2020 rivelatosi sia troppo permissivo (contenuto 2022--2023
obsoleto) sia troppo aggressivo (scartava PDF recenti privi di alternative). URL
accumulativi (pubblicazioni, premi): cutoff 2020, ogni anno distinto. PDF versionati per
anno (schede SUA, regolamenti, piani di studio): logica \emph{latest-version} a due
passate, mantiene solo l'anno massimo $\ge 2020$. URL HTML generici (docenti, corsi,
bandi): cutoff 2025 con riconoscimento pattern \texttt{YYYY-YY}. PDF individuali: cutoff
2020 con delega a latest-version. I documenti scartati sono salvati in
\texttt{dropped\_temporal\_filter.json} per audit.
...
mantiene la codifica simmetrica di E5 (\texttt{query:}/\texttt{passage:}). L'evoluzione
...
0,50 (\code{RETRIEVER\_SCORE\_THRESHOLD}, alcune run C2--C5 con 0,45 per l'ablation). Il
bi-encoder finale è \texttt{qwen/qwen3-embedding-8b} via OpenRouter, con fallback locale
\texttt{multilingual-e5-base}; \texttt{baai/bge-m3} è stato valutato ma non scelto perché
il sistema non usa ricerca ibrida sparse+dense.
...
veniva quasi mai chiamato dal 120b, che gestiva direttamente contesti estesi. La sua
variante \code{summarize\_history} (compressione della history al decimo turno) mostrava
buona qualità di sintesi ma un problema di compliance: dopo la chiamata il modello
generava una risposta generica senza procedere con \code{retrieve()}, interpretando il
summary come contesto sufficiente --- la sequenza multi-tool nello stesso turno risultava
inaffidabile per questo modello. Entrambe le varianti sono state rimosse in favore della
sliding window (Sezione~\ref{sec:core-rag}).
...
\textbf{AGENT\_SYSTEM\_PROMPT.} Il prompt adotta \emph{role prompting} (identità esplicita
di assistente DIEM) e una struttura in step numerati (\emph{Step~1: tool use},
\emph{Step~2: context validation}, \emph{Step~3: generate answer}), con un sotto-protocollo
di self-verification del contesto allo Step~2. I tag strutturati \texttt{[KNOWLEDGE\_GAP]}
e \texttt{[FUORI\_SCOPE]} (\emph{structured output prompting}) permettono al layer
applicativo di rilevare i rifiuti senza parsing euristico. Diverse regole derivano da
comportamenti osservati empiricamente: ``NEVER EMPTY'' dopo che il modello emetteva il
solo tag senza spiegazione; soglia del tag knowledge-gap allentata per ridurre falsi
negativi su contesto parziale; regola sui ruoli accademici dopo aver osservato docenti
RTD/B introdotti erroneamente come ``ricercatori''.

\textbf{REWRITE\_PROMPT.} Combina \emph{instruction following} con \emph{few-shot
prompting}: regole esplicite sui casi edge (non sostituire la domanda corrente, risolvere
pronomi, preservare entità, iniettare l'anno accademico) seguite da esempi input/output
annotati, necessari perché le regole in linguaggio naturale non bastano a distinguere un
follow-up ambiguo da una domanda indipendente. Il prompt finale estende le regole iniziali
con casi mirati (voto di laurea, aree di ricerca, laboratori, piano di studi, challenge di
conferma) emersi empiricamente, riducendo la variabilità del modello lightweight.
...
mirato); una regola di \emph{chain-of-thought} \cite{wei2022chain} allo Step~3, che
obbliga l'agente a esplicitare ogni fattore intermedio (V, FC, bonus) prima del risultato
finale, introdotta dopo aver osservato errori sull'FC non rilevabili nel solo voto finale.
...
\code{golden\_set\_en.json}); le domande con \code{reference} esplicita alimentano le
metriche Ragas ground-truth, le altre solo le metriche no-reference. È stato costruito
manualmente con domande realistiche sui contenuti DIEM; le categorie
\texttt{out\_of\_scope} e \texttt{robustness} sono state aggiunte in un secondo momento,
dopo che i primi test manuali avevano evidenziato comportamenti inattesi (scope guard
troppo/poco permissivo, retrieval drift sulle challenge).
...
Per ogni run, il runner produce file strutturati (\code{run.log}, \code{per\_question.json},
\code{ragas\_metrics.csv/json}, \code{scope\_awareness.json}, \code{robustness.json},
\code{summary.md}) e una cache su disco a tre modalità (\code{off}/\code{use}/\code{refresh})
chiavata su modello, temperatura, history e domanda, per evitare che run con
configurazioni diverse condividano cache.
...
l'ipotesi che trafilatura perdesse contenuto significativo sulle pagine JavaScript-heavy
del sito DIEM, inducendo il modello ad allucinare. Lo studio comparativo con Crawl4AI si
è articolato su tre livelli (Tabella~\ref{tab:migration}): qualità di estrazione su 10 URL
rappresentativi, valore del rendering JS (Playwright vs.\ fetch statico) e prestazioni
Q/A end-to-end sull'intera pipeline RAG con golden set esteso a 30 domande.

Al Livello A, Crawl4AI estrae il 39\% di testo in più (2319 vs.\ 3231 caratteri), ma il
gap è trainato quasi interamente da due pagine JS-heavy dove trafilatura ricade in un
fallback quasi nullo; su pagine statiche i due estrattori sono comparabili, e Crawl4AI
presenta un noise ratio leggermente superiore per boilerplate residuo. Al Livello B, il
rendering dinamico aggiunge solo il 9\% di contenuto sulla homepage (la pagina più
dinamica del corpus), non giustificando la complessità operativa di un browser headless.
Al Livello C, trafilatura ottiene un overlap lessicale medio di 0,384 contro 0,288 di
Crawl4AI (distribuzione 18-9-3, test binomiale $p\approx1{,}5\%$), trainato dalle domande
su fatti atomici precisi (numeri, codici, P.IVA) dove Crawl4AI introduce più rumore.

\begin{table}[htbp]
\centering
\caption{Riepilogo dello studio comparativo trafilatura vs.\ Crawl4AI.}
\label{tab:migration}
\small
\begin{tabularx}{\textwidth}{lX}
\toprule
\textbf{Livello} & \textbf{Esito} \\
\midrule
A --- Qualità estrazione & Crawl4AI $+$39\% testo, ma trainato da 2 pagine JS-heavy; comparabili su pagine statiche. \\
B --- Rendering JS & $+$9\% contenuto su homepage; non giustifica un browser headless. \\
C --- Q/A end-to-end (30 domande) & Trafilatura 0,384 vs.\ Crawl4AI 0,288 overlap lessicale ($p\approx1{,}5\%$), superiore su fatti atomici. \\
\bottomrule
\end{tabularx}
\end{table}
...
JS-heavy non si trasferisce al Q/A end-to-end perché il corpus ampio fornisce ridondanza
informativa, mentre trafilatura resta superiore per il caso d'uso primario del chatbot:
domande su fatti atomici precisi.
...
\noindent La soglia indicata in Tabella~\ref{tab:configs} descrive gli snapshot
usati nei run di valutazione. Nel codice corrente il parametro è controllato da
\code{RETRIEVER\_SCORE\_THRESHOLD}; in assenza di override nell'ambiente, il default
rimane 0,50.

Le transizioni sono quattro. C1$\to$C2: LLM locale sostituito da un modello cloud di
larga scala, con reranker Cohere Rerank-4-Pro, soglia abbassata a 0.45 e CrossK aumentato
...
(2000/200 | 400/50) con i raffinamenti agentici del Capitolo~4 (rewrite retry-aware,
sliding window, verifica del regolamento di laurea, precedenza accademica), che agiscono
sul comportamento del grafo senza alterare il retrieval.
...
Le altre metriche non usano un giudice LLM. L'incertezza intrinseca del giudice rende
comunque indispensabile affiancare test manuali sistematici: la scelta di C5 come
configurazione finale è stata supportata da entrambi.
...
chunk figlio senza duplicarlo nei parent restituiti al modello. Lo studio comparativo
trafilatura/Crawl4AI ha inoltre mostrato che i miglioramenti di qualità dell'estrattore a
livello di singola pagina non si trasferiscono necessariamente al Q/A end-to-end quando
la pipeline include ridondanza informativa: il guadagno più significativo è venuto
dall'ottimizzazione della pipeline stessa (flat k=3 $\to$ ParentRetriever + cross-encoder:
...
\begin{itemize}
  ...
  \item \textbf{Limiti di lettura dei PDF.} Estrazione testuale, non OCR visuale: nei
    PDF tabellari il testo può essere letto per colonne invece che per righe (es.\
    campi di una riga tabellare mescolati), e i PDF manoscritti/scansionati restano
    illeggibili --- motiva il lavoro futuro su OCR dedicato.
...
\begin{itemize}
  ...
  \item \textbf{Crawling con agent browser.} Un agent browser al posto del crawler HTML
    statico accederebbe a contenuto JavaScript, accordion e sezioni lazy-loaded oggi non
    indicizzabili.
\end{itemize}
