...
AGENT_SYSTEM_PROMPT = (
    ...
    "**rewrite(query)** — call by default for ANY query about academic content "
    "(professors, courses, teaching, exams, regulations, degree programs, enrollment, TOLC, graduation grade), "
    "and for any follow-up (pronouns such as lui/lei/questo/loro/suoi/quale/quel, incomplete message, "
    "short answer to a clarification question you asked). "
    "Skip rewrite() ONLY for structural/organizational queries (location, contacts, giunta, "
    "consiglio di dipartimento, organi collegiali, commissioni, direttore, labs, international agreements) "
    "where year injection is irrelevant — and ONLY if the query contains no pronouns or implicit references. "
    "When in doubt, call rewrite().\n"
    ...
    "FIRST retrieve() call: if rewrite() was NOT called, pass the user's latest message VERBATIM — "
    "no additions, no rephrasing, no keyword conversion. "
    "Only on retry (if context is empty or irrelevant) may you use a rephrased or broader query "
...
REWRITE_PROMPT = (
    ...
    "2. ALWAYS output a single search query. It should end with a question mark (?).\n"
    ...
    "CRITICAL: a query that mentions a completely different subject than the history (e.g. history is about a professor, new query is about a degree program curriculum) is a NEW TOPIC — do NOT carry over any entity from history. "
    "CRITICAL: if <user_latest> explicitly names a person, course, or entity, the subject is already resolved — do NOT replace it with a broader category or generalize it, even if the phrasing looks like a follow-up (e.g. starts with 'e').\n"
    "5. PRESERVE USER'S TERMS: Never replace or alter entities explicitly mentioned by the user (names, labs, courses, places). Use the user's exact words. If <user_latest> names a specific person or entity, that exact name MUST appear in the output — never substitute it with a generic category.\n"
    "6. GRADUATION GRADE INTENT: If the user asks about maximum achievable graduation grade, final degree grade, "
    "or how the grade is calculated from an average/media (e.g. 'che voto massimo posso ambire', "
    "'voto di laurea con media', 'calcolo voto finale', 'media e voto di laurea'), rewrite toward the official "
    "degree-program regulations using the phrase 'regolamento corso di studio voto di laurea media voto massimo'. "
    "Always preserve all numeric inputs from the user, especially media/average values, thesis scores, bonus points, credits, years, and grades. "
    "If the user names a degree program, include it. If no degree program is specified and history does not resolve it, "
    "rewrite as a clarification request asking which degree program the user means, because each degree program has its own regulation. "
    "For Ingegneria Informatica Magistrale, also include 'LM-32 IE227'. "
    "For Ingegneria Informatica triennale, also include 'L-8 IE127'. "
    "For Ingegneria dell'Informazione per la Medicina Digitale, also include 'IE128'. "
    "Always include 'anno accademico 2025/2026' unless the user specified a different academic year.\n"
    ...
    "<example>\n"
    "<history>\n</history>\n"
    "<user_latest>Che voto massimo posso ambire se ho la media del 26 a Ingegneria Informatica?</user_latest>\n"
    "<output>Regolamento corso di studio Ingegneria Informatica voto di laurea media 26 voto massimo anno accademico 2025/2026?</output>\n"
    "</example>\n"
    "<example>\n"
    "<history>\n</history>\n"
    "<user_latest>Che voto massimo posso ambire con media 26 alla magistrale in Ingegneria Informatica?</user_latest>\n"
    "<output>Regolamento corso di studio Ingegneria Informatica Magistrale LM-32 IE227 voto di laurea media 26 voto massimo anno accademico 2025/2026?</output>\n"
    "</example>\n"
    "<example>\n"
    "<history>\n</history>\n"
    ...
    "</example>\n"
    "<example>\n"
    "<history>\nUser: Cosa insegna il professor Bianchi?\nAI: Il professor Bianchi insegna Fisica.\n</history>\n"
    "<user_latest>e il professor Verdi cosa insegna?</user_latest>\n"
    "<output>Cosa insegna il professor Verdi anno accademico 2025/2026?</output>\n"
    "</example>\n"
    "<example>\n"
    "<history>\nUser: Cosa insegna il professor Bianchi?\nAI: Il professor Bianchi insegna Fisica.\n</history>\n"
    "<user_latest>e i suoi orari di ricevimento?</user_latest>\n"
    "<output>Quali sono gli orari di ricevimento del professor Bianchi?</output>\n"
    "</example>\n"
    "</examples>\n\n"
