...
AGENT_SYSTEM_PROMPT = (
    ...
    "- About which courses a professor teaches (e.g. 'che corsi insegna', 'quali corsi tiene', "
    "'cosa insegna', 'insegnamenti di', 'corsi erogati da') — even if the professor name is already explicit\n"
    ...
    "You may skip rewrite() if the query is already self-contained — especially for structural or "
    "organizational queries (department organs, giunta, consiglio di dipartimento, organi collegiali, "
    "commissioni, direttore, labs, contacts, location, international agreements) where year injection "
    "would be wrong and the query needs no expansion; and for queries about degree program curricula or "
    "study plans (piano di studi, corsi del curriculum, esami della magistrale/triennale) that already "
    "specify the degree program and need no entity resolution from history. "
...
REWRITE_PROMPT = (
    ...
    "6. TEACHER COURSES INTENT: If the user asks which courses a specific named professor teaches "
    "(e.g. 'quali corsi insegna [nome]', 'che corsi tiene [nome]', 'cosa insegna [nome]', 'insegnamenti di [nome]', "
    "'corsi erogati da [nome]'), rewrite the query using the retrieval phrase "
    "'Corsi insegnati <GivenName SURNAME>'. "
    "This rule ONLY applies when a professor's name is present or resolvable from history. "
    "Do NOT apply this rule for queries about study plans, curricula, or degree program courses "
    "(e.g. 'corsi del curriculum AI', 'piano di studi', 'quali esami ci sono in', 'corsi della magistrale'). "
    "Do NOT add an academic year for this intent unless the user explicitly specified one.\n"
    "7. GRADUATION GRADE INTENT: If the user asks about maximum achievable graduation grade, final degree grade, "
    ...
    "8. YEAR INJECTION: If the query concerns academic content (courses, regulations, degree programs, "
