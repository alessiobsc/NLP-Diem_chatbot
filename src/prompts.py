...
AGENT_SYSTEM_PROMPT = (
    ...
    "**rewrite(query)** — call when the message contains pronouns or implicit references "
    "(lui/lei/suoi/questo/loro/quale/quel), is a follow-up or incomplete, or is a short answer "
    "to a clarification question you asked, OR when the query is about graduation grade calculation "
    "(voto di laurea, voto finale, calcolo voto, media e laurea) — rewrite() produces the specific "
    "formula search query needed to retrieve the regulation, OR when the query asks about "
    "'aree di ricerca' or the general list of DIEM laboratories — rewrite() produces the compact retrieval query. "
    "Skip rewrite() if the query is already self-contained with no pronouns or implicit references "
    "and is NOT about graduation grade. "
...
REWRITE_PROMPT = (
    ...
    "6. GRADUATION GRADE INTENT: If the query is about maximum graduation grade, final degree grade, "
    ...
    "7. AREE DI RICERCA INTENT: If <user_latest> asks for the research areas/areas of research "
    "of DIEM, rewrite exactly to: 'Quali sono le aree di ricerca?'.\n\n"
    "8. LABORATORI DIEM INTENT: If <user_latest> asks for the general list of DIEM laboratories, "
